        ...
        <section class="relative hidden min-h-full overflow-hidden bg-[linear-gradient(145deg,#16382b_0%,#24553f_50%,#2f7a53_100%)] p-10 text-white lg:flex lg:flex-col lg:justify-between">
          ...
          <div class="relative grid gap-4 sm:grid-cols-2">
            <div class="rounded-[24px] border border-white/15 bg-white/10 p-5 backdrop-blur-sm">
              ...
              <p class="text-sm font-black uppercase tracking-[0.2em] text-white/60">{{ t('admin.login.secureAccessTitle') }}</p>
              <p class="mt-2 text-sm leading-6 text-white/78">{{ t('admin.login.secureAccessDesc') }}</p>
            </div>
            ...
            <div class="rounded-[24px] border border-white/15 bg-white/10 p-5 backdrop-blur-sm">
              ...
              <p class="text-sm font-black uppercase tracking-[0.2em] text-white/60">{{ t('admin.login.matchdayReadyTitle') }}</p>
              <p class="mt-2 text-sm leading-6 text-white/78">{{ t('admin.login.matchdayReadyDesc') }}</p>
            </div>
          </div>
        </section>
            ...
            <div class="rounded-[28px] border border-brand-line/80 bg-[#fffdf8]/92 p-6 shadow-[0_18px_45px_rgba(18,55,42,0.08)] sm:p-8">
              ...
              <form class="space-y-5" @submit.prevent="handleLogin">
                ...
                <label class="block space-y-2">
                  <span class="px-1 text-[11px] font-black uppercase tracking-[0.22em] text-brand-slate">{{ t('admin.login.emailLabel') }}</span>
                ...
                <label class="block space-y-2">
                  <span class="px-1 text-[11px] font-black uppercase tracking-[0.22em] text-brand-slate">{{ t('admin.login.passwordLabel') }}</span>
                  <div class="group relative">
                    <div class="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-brand-slate transition-colors group-focus-within:text-brand-court">
                      <LockKeyhole :size="18" />
                    </div>
              ...
              <div class="mt-6 flex items-center justify-between gap-4 rounded-2xl border border-brand-line bg-brand-sand/70 px-4 py-3 text-sm">
                <div>
                  <p class="font-black text-brand-ink">{{ t('admin.login.privateWorkspace') }}</p>
                  <p class="mt-1 text-brand-slate">{{ t('admin.login.privateWorkspaceDesc') }}</p>
                </div>
