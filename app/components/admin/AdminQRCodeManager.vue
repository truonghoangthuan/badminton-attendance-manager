...
<template>
  <UIGlassCard class="space-y-4">
    <div
      v-if="qrAmount === 0"
      class="rounded-[28px] border border-dashed border-brand-line bg-brand-sand/60 px-6 py-8 text-center text-brand-slate"
    >
      <QrCode :size="30" class="mx-auto opacity-40" />
    </div>

    <div v-else class="rounded-[28px] bg-white p-4">
      <div class="group relative mx-auto w-full max-w-sm">
        <img :src="qrCodeUrl" alt="VietQR payment code" class="aspect-square w-full object-contain" />
        <button
          type="button"
    ...
    <Teleport to="body">
      ...
      >
        <div class="relative w-full max-w-xl rounded-[40px] bg-white p-6 shadow-2xl" @click.stop>
          ...
          <img :src="qrCodeUrl" alt="Expanded VietQR payment code" class="aspect-square w-full object-contain" />
        </div>
      </div>
    </Teleport>
  </UIGlassCard>
